const parser = require('../utils/csv_parser')

describe('Validator tests', () => {
  describe('Time validator', () => {
    test('validateTime return true with valid time', () => {
      const valid = parser.validateTime('2021-06-01T00:05:46')
      expect(valid).toBe(true)
    })

    test('validateTime returns false with undefined time', () => {
      const valid = parser.validateTime(undefined)
      expect(valid).toBe(false)
    })

    test('validateTime returns false with empty string', () => {
      const valid = parser.validateTime('')
      expect(valid).toBe(false)
    })

    test('validateTime returns false with invalid format', () => {
      const valid = parser.validateTime('20210601T000546')
      expect(valid).toBe(false)
    })

    test('validateTime returns false with impossible date 1', () => {
      const valid = parser.validateTime('2021-50-01T00:05:46')
      expect(valid).toBe(false)
    })

    test('validateTime returns false with impossible date 2', () => {
      const valid = parser.validateTime('2021-06-01T77:77:77')
      expect(valid).toBe(false)
    })
  })

  describe('Id validator', () => {
    test('validateId returns true with valid id', () => {
      const valid = parser.validateId('10')
      expect(valid).toBe(true)
    })

    test('validateId returns false with undefined id', () => {
      const valid = parser.validateId(undefined)
      expect(valid).toBe(false)
    })

    test('validateId returns false with non-number', () => {
      const valid = parser.validateId('id')
      expect(valid).toBe(false)
    })

    test('validateId returns false with float number', () => {
      const valid = parser.validateId('547.1')
      expect(valid).toBe(false)
    })

    test('validateId returns false with negative number', () => {
      const valid = parser.validateId('-10')
      expect(valid).toBe(false)
    })
  })

  describe('Name validator', () => {
    test('validateStationName returns true with valid name', () => {
      const valid = parser.validateStationName('Käpylän asema')
      expect(valid).toBe(true)
    })

    test('validateStaionName returns false with undefined name', () => {
      const valid = parser.validateStationName(undefined)
      expect(valid).toBe(false)
    })

    test('validateStationName returns false with empty string', () => {
      const valid = parser.validateStationName('')
      expect(valid).toBe(false)
    })
  })

  describe('Distance validator', () => {
    test('validateDistance returns true with valid distance', () => {
      const valid = parser.validateDistance('11')
      expect(valid).toBe(true)
    })

    test('validateDistance returns false with undefined distance', () => {
      const valid = parser.validateDistance(undefined)
      expect(valid).toBe(false)
    })

    test('validateDistance return false with negative distance', () => {
      const valid = parser.validateDistance('-10')
      expect(valid).toBe(false)
    })

    test('validateDistance returns false with non-number string', () => {
      const valid = parser.validateDistance('distance')
      expect(valid).toBe(false)
    })

    test('validateDistance returns false with float-number', () => {
      const valid = parser.validateDistance('12.12')
      expect(valid).toBe(false)
    })

    test('validateDistance returns false with distance less than ten', () => {
      const valid = parser.validateDistance('9')
      expect(valid).toBe(false)
    })

    test('validateDistance returns false with negative integer', () => {
      const valid = parser.validateDistance('-9')
      expect(valid).toBe(false)
    })
  })

  describe('Duration validator', () => {
    test('validateDuration returns true with correct input', () => {
      const valid = parser.validateDuration('2021-05-31T23:57:25', '2021-06-01T00:05:46', 500)
      expect(valid).toBe(true)
    })

    test('validateDuration returns false with undefined input', () => {
      const valid = parser.validateDuration(undefined, undefined, undefined)
      expect(valid).toBe(false)
    })

    test('validateDuration returns false with non-number string', () => {
      const valid = parser.validateDuration('2021-05-31T23:57:25', '2021-06-01T00:05:46', 'duraiton')
      expect(valid).toBe(false)
    })

    test('validateDuration returns false with negative integer', () => {
      const valid = parser.validateDuration('2021-05-31T23:57:25', '2021-06-01T00:05:46', '-10' )
      expect(valid).toBe(false)
    })

    test('validateDuration returns false with duration less than 10 seconds', () => {
      const valid = parser.validateDuration('2021-05-31T00:00:00', '2021-06-01T00:00:09', '9')
      expect(valid).toBe(false)
    })

    test('validateDuration returns false when time difference is too small compared to duration', () => {
      const valid = parser.validateDuration('2021-05-31T00:00:00', '2021-06-01T00:00:09', '11')
      expect(valid).toBe(false)
    })

    test('validateDuration returns false when time difference is too large compared to duration', () => {
      const valid = parser.validateDuration('2021-05-31T00:00:00', '2021-06-01T00:00:20', '10')
      expect(valid).toBe(false)
    })

    test('validateDuration returns false when departure_time is later than return_time', () => {
      const valid = parser.validateDuration('2021-06-01T00:05:46', '2021-05-31T23:57:25', '500')
      expect(valid).toBe(false)
    })
  })
  describe('Line validator', () => {
    test('validateLine returns true with valid input', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:46,094,Laajalahden aukio,100,Teljäntie,2043,500')
      expect(valid).toBe(true)
    })

    test('validateLine returs false with undefined', () => {
      const valid = parser.validateLine(undefined)
      expect(valid).toBe(false)
    })

    test('validateLine returns false with empty string', () => {
      const valid = parser.validateLine('')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid departure_time', () => {
      const valid = parser.validateLine('departure_time,2021-06-01T00:05:46,094,Laajalahden aukio,100,Teljäntie,2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid return_time', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,return_time:46,094,Laajalahden aukio,100,Teljäntie,2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid departure_station_id', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:-46,094,Laajalahden aukio,100,Teljäntie,2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid departure_station_name', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:46,094,,100,Teljäntie,2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid return_station_id', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:46,094,Laajalahden aukio,-100,Teljäntie,2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid return_station_name', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:46,094,Laajalahden aukio,100,,2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid distance', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:46,094,Laajalahden aukio,100,Teljäntie,-2043,500')
      expect(valid).toBe(false)
    })

    test('validateLine returns false with invalid duration', () => {
      const valid = parser.validateLine('2021-05-31T23:57:25,2021-06-01T00:05:46,094,Laajalahden aukio,100,Teljäntie,2043,-500')
      expect(valid).toBe(false)
    })
  })
})

describe('Datapoint parser tests', () => {
  test('parseDatapoint returns correct data', () => {
    const datapoint = parser.parseDatapoint('2021-05-31T23:57:25,2021-06-01T00:05:46,094,Laajalahden aukio,100,Teljäntie,2043,500')
    expect(datapoint.departure_time).toBeDefined()
    expect(datapoint.return_time).toBeDefined()
    expect(datapoint.departure_station_id).toBeDefined()
    expect(datapoint.departure_station_name).toBeDefined()
    expect(datapoint.return_station_id).toBeDefined()
    expect(datapoint.return_station_name).toBeDefined()
    expect(datapoint.distance).toBeDefined()
    expect(datapoint.duration).toBeDefined()

    expect(typeof datapoint.departure_time).toBe('string')
    expect(typeof datapoint.return_time).toBe('string')
    expect(typeof datapoint.departure_station_id).toBe('number')
    expect(typeof datapoint.departure_station_name).toBe('string')
    expect(typeof datapoint.return_station_id).toBe('number')
    expect(typeof datapoint.return_station_name).toBe('string')
    expect(typeof datapoint.distance).toBe('number')
    expect(typeof datapoint.duration).toBe('number')
  })
})

describe('csvParser tests', () => {
  test('csvParser returns correct array with file containing only valid data', async() => {
    const data = await parser.csvParser('./tests/valid_data.csv')
    expect(data).toBeDefined()
    expect(data.length).toBe(15)
    const sample = data[0]
    expect(sample).toEqual({
      departure_time: '2021-05-31T23:57:25',
      return_time: '2021-06-01T00:05:46',
      departure_station_id: 94,
      departure_station_name: 'Laajalahden aukio',
      return_station_id: 100,
      return_station_name: 'Teljäntie',
      distance: 2043,
      duration: 500
    })
  })

  test('csvParser throws error with undefined filename', () => {
    () => expect(() => parser.csvParser(undefined)).toThrow('Missing or invalid filename')
  })

  test('csvParser throws error with empty filename', () => {
    () => expect(() => parser.csvParser('')).toThrow('Missing or invalid filename')
  })

  test('csvParser returns empty array with file containing only incorrect data', async() => {
    const data = await parser.csvParser('./tests/invalid_data.csv')
    expect(data.length).toBe(0)
  })
})
